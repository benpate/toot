...
[![Go Reference](https://pkg.go.dev/badge/github.com/benpate/toot.svg)](https://pkg.go.dev/github.com/benpate/toot)
...
Toot implements a REST API server that matches the [Mastodon REST API](https://docs.joinmastodon.org/api/). It plugs into your existing application seamlessly (using a [router adapter](#routers)), making your Go application accessible to any Mastodon-compatible client app.

## Why would I do this?

Most client apps use the Mastodon REST API instead of the official ActivityPub Client-to-Server (C2S) API. However, the Mastodon API is very large, with a huge number of routes, handlers, and scopes required to support. Toot provides a consistent catalog of all of these values, and handles lots of busywork -- matching the specific routes, authorization, parameters, and results that Mastodon requires -- so that you can write happy little API handlers like this:
...
However, at this point, feedback from other developers will be immensely helpful in making Toot a useful tool for the Go community. Please feel free to open issues, or submit pull requests. We're all in this together! 🎺
